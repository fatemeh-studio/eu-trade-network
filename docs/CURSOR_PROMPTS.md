...
Drive the whole build with these, in order, using the **Cursor Agent** on **Opus 4.8**.
Paste one prompt, let it finish, then **commit yourself** (the agent does not commit — it
ends by reminding you and suggesting a message) before pasting the next. Every prompt tells
the agent to follow `AGENTS.md` and to end with `ruff` + `basedpyright` + `pytest` green.
...
3b) Configure the nbstripout git filter for this clone: `nbstripout --install --attributes .gitattributes` (keeps notebook outputs on disk but strips them from git).
...
Report python/ruff/basedpyright/pytest versions. Set P0 to done in PROGRESS.md, then remind me to commit `chore: bootstrap env and tooling` myself.
```
...
Acceptance: ruff + basedpyright + pytest pass. Update PROGRESS.md, then remind me to commit `feat: BACI data loader and edge-list pipeline` myself.
...
Acceptance: NB01 runs top-to-bottom; all checks green; figure saved. Update PROGRESS.md and the README hero path, then remind me to commit `feat: graph construction, DuckDB storage, flow map (NB01)` myself.
```
...
Acceptance: all green; NB02 runs. Update PROGRESS.md and the RQ1 finding, then remind me to commit `feat: centrality, disparity-filter backbone, rich-club (NB02)` myself.
```
...
Acceptance: all green; NB03 runs; PyVis HTML produced. Update PROGRESS.md and the RQ2 finding, then remind me to commit `feat: Louvain communities and interactive network (NB03)` myself.
```
...
Acceptance: all green; NB04 runs. Update PROGRESS.md and the RQ3 finding, then remind me to commit `feat: resilience simulation and curves (NB04)` myself.
```
...
Acceptance: NB05 runs; all green. Update PROGRESS.md and the RQ4 finding, then remind me to commit `feat: energy (HS-27) subnetwork bridge (NB05)` myself.
...
3) Ensure figures/headline/ has one clean PNG per RQ; notebooks run top-to-bottom (outputs stay on disk but are stripped from git by the nbstripout filter).
4) Final green check: ruff, basedpyright, pytest, quarto render.
5) Remind me to commit `docs: report, README findings, headline figures` and then tag `git tag v0.1.0` myself.
